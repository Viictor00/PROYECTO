...
//Estructura necesaria acceso excluyente
...
void *AtenderCiente (void *socket)
{
	...
	while (terminar ==0)
	{
		...
		else if (codigo == 7) // El usuario quiere saber el numero de servicios a la base de datos 
		{
			sprintf (respuesta, "El numero de servicios es %d",contador); // devolvemos numero de servicios 
		}
